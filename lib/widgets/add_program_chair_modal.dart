...
class AddProgramChairModal extends StatefulWidget {
  final bool isOpen;
  final VoidCallback onClose;
  final Function(Map<String, dynamic>) onAddProgramChair;
  final List<dynamic> colleges;

  const AddProgramChairModal({
    super.key,
    required this.isOpen,
    required this.onClose,
    required this.onAddProgramChair,
    required this.colleges,
  });

  @override
  State<AddProgramChairModal> createState() => _AddProgramChairModalState();
}

class _AddProgramChairModalState extends State<AddProgramChairModal> {
  ...
  final String _selectedRole = 'programchairperson';
  ...
  Future<void> _handleAddProgramChair() async {
    ...
    try {
      ...
      if (mounted) {
        widget.onAddProgramChair(programChairData);
        _showSuccessSnackBar('Program Chair added successfully!');
        _resetForm();
        widget.onClose();
      }
    } catch (e) {
      if (mounted) {
        _showErrorSnackBar('Failed to add program chair: $e');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _resetForm() {
    ...
    _selectedCollege = '';
    // Reset courses when form is cleared
    _generatePassword();
  }
          ...
          children: [
                      ...
                      overflow: TextOverflow.ellipsis,
                              ...
                              onChanged: (value) {
                                setState(() {
                                  ...
                                  // Reset courses when college changes
                                });
                                _loadCourses();
                              },
                              label: 'College *',
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 16),
                ElevatedButton(
                  onPressed: _isLoading ? null : _handleAddProgramChair,
